...
for (j = 0; j < filelist.length; j++) {
	if (endsWith(filelist[j], ".tif")){
		...
		img=getTitle();
		b=split(filelist[j],"_");
		number=lengthOf(b[0]+b[1]+b[2])+2;
		imgname=substring(filelist[j],number,lengthOf(filelist[j])-3)+"tif";
		roiname="ratio"+substring(filelist[j],number,lengthOf(filelist[j])-4)+"_.zip";
