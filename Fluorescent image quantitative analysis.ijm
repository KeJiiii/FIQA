...
for (times = 0; times < 4 ; times++) {
	...
	Dialog.create("Fluorescent Image Quantitative Analysis  ");
		...
		Dialog.addMessage("3. Remove skins");
		Dialog.addMessage("4. Analysis and data export");
	...
	function get_particle(select_path,son_file_list,species) { 
		for (l = 0; l < son_file_list.length; l++){
			if (startsWith(son_file_list[l],species) && endsWith(son_file_list[l],"tif")){
				...
				// select which channel to be the reference
				
				if (select_channel=="Ch1") run("Duplicate...", "duplicate");
				if (select_channel=="Ch2") run("Duplicate...", "duplicate");
				if (select_channel=="color_ratio") selectWindow("Color Ratio");
				...
				selectWindow(Ch1);
				roiManager("Show All");
				run("Set Measurements...", "area mean min redirect=None decimal=8");
				roiManager("Measure");
	...
	function remove_zskin(fix,chann,move,save_dir,threshold,Fill,Erode) { 
		mergestring="";
		open(fix); 
		reference=getTitle();
		...
		mask=getTitle();
		selectWindow(mask);
		run("Duplicate...", "duplicate channels="+chann);
		run("8-bit");
		mask1 = getTitle();
		...
		saveAs("tiff", save_dir+"Manual_"+reference1);
		close("*");
	}
	...
	function user_input(){
		...
		if (user_choice==4){
			...
			Dialog.create("Remove skin parameters");
				...
				Dialog.addNumber("which channel will be the reference", 2);
				Dialog.addNumber("minimum threshold", 10);
					...
					if(rem=="no"){
						for (l = 0; l < son_file_list.length; l++){
							if (startsWith(son_file_list[l],"Sub") && endsWith(son_file_list[l],".tif")){
								...
								remove_zskin(fix,chann,move,save_dir,threshold,Fill,Erode);
							}
							close("*");
						}
					}
		...
		*/
		...
		if (user_choice==4){
			...
			if (species_results=="cell"){species="Sub_";}
			if (species_results=="fish"){species="rem_";}

			if (user_dirc==2){
				for (f = 0; f < fa_file_list.length; f++){
					select_path = fa_path+fa_file_list[f];
					son_file_list=getFileList(select_path);
					get_particle(select_path,son_file_list,species);
				}
			}
			else{
				select_path = fa_path;
